...
def validate_release_sheet_docx_blob(blob_name: str, profile: str = "system_release_sheet") -> ValidationResult:
    ...
    def passed(cid):
        checks.append(ValidationCheck(check_id=cid, status="passed"))

    def failed(cid, msg):
        checks.append(ValidationCheck(check_id=cid, status="failed", message=msg))

    def warning(cid, msg):
        checks.append(ValidationCheck(check_id=cid, status="warning", message=msg))
    ...
    status = (
        ...
        else ("warning" if any(c.status == "warning" for c in checks) else "passed")
    )
