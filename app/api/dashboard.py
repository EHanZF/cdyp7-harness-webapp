...
@router.get("/files")
def files_list():
    # use wrapper name so tests can monkeypatch `dashboard.list_file_entries`
...
def files_annotate(file_id: str, payload: dict[str, Any], request: Request):
    ...
    try:
        ...
        raise HTTPException(status_code=404, detail="not_found")
    append_audit_event(event_type="file.annotate", actor=actor, metadata={"file_id": file_id, "updates": payload})
    return {"status": "ok", "file": entry}
...
def ingest_start(payload: dict[str, Any], request: Request):
    ...
    append_audit_event(event_type="ingest.start", actor=actor, metadata={"job_id": job_id, "file_count": len(file_ids)})
...
def alm_action(system: str, artifact: str, payload: dict[str, Any], request: Request):
    ...
    append_audit_event(
        ...
        actor=actor,
        metadata={"system": system, "artifact": artifact, "action": payload.get("action")},
    )
...
def audit_list(limit: int = 200):
    # call without args so tests can monkeypatch list_audit_events() easily
...
def task_state(task_id: str):
    # use file_index.get_task_state wrapper so tests can monkeypatch dashboard.get_task_state
    try:
...
def task_restpoint(task_id: str, payload: dict[str, Any], request: Request):
    ...
    append_audit_event(event_type="task.rest_point", actor=actor, metadata={"task_id": task_id})
    return {"status": "ok"}
